...
{#if popper_show}
  <div bind:this={popper} class="{popper_class} {has_arrow ? PAGE_STYLE.POPPER_PLACEMENT[current_placement] : ''}" class:hidden={!popper_class_show}>
    {#if has_arrow}
      <div
        id="arrow"
        class="self-center border-x-[6px] border-x-transparent border-b-[8px] border-b-white {PAGE_STYLE.ARROW_PLACEMENT[current_placement] ?? ''}"
